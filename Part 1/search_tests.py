...
class TestSearch(TestCase):
    ...
    def test_integration_title_length(self, input_mock):
        # Testing title length filter for articles with titles of 20 characters or fewer.
        keyword = 'music'
        advanced_option = 1
        max_length = 20
        output = get_print(input_mock, [keyword, advanced_option, max_length])
        # Expected output with articles having titles shorter than or equal to 20 characters.
        expected = print_basic() + keyword + '\n' + print_advanced() + str(advanced_option) + '\n' + print_advanced_option(advanced_option) + str(max_length) + '\n\nHere are your articles: [\'Noise (music)\', \'1922 in music\', \'1986 in music\', \'2009 in music\', \'Rock music\', \'1936 in music\', \'1996 in music\', \'2006 in music\', \'2007 in music\', \'2008 in music\']\n'
        self.assertEqual(output, expected)

    @patch('builtins.input')
    def test_integration_article_count(self, input_mock):
        # Testing article count with a limit of 5 articles to be returned.
        keyword = 'music'
        advanced_option = 2
        count = 5
        output = get_print(input_mock, [keyword, advanced_option, count])
        # Expected output with only the first 5 articles returned.
        expected = print_basic() + keyword + '\n' + print_advanced() + str(advanced_option) + '\n' + print_advanced_option(advanced_option) + str(count) + '\n\nHere are your articles: [\'List of Canadian musicians\', \'French pop music\', \'Noise (music)\', \'1922 in music\', \'1986 in music\']\n'
        self.assertEqual(output, expected)

    @patch('builtins.input')
    def test_integration_random_article(self, input_mock):
        # Testing random article selection by choosing the article at index 1.
        keyword = 'music'
        advanced_option = 3
        index = 1
        output = get_print(input_mock, [keyword, advanced_option, index])
        # Expected output displaying the article at index 1.
    ...
    def test_integration_favorite_article(self, input_mock):
        # Testing favorite article functionality with the favorite article 'Rock music'.
        keyword = 'music'
        advanced_option = 4
        favorite = 'Rock music'
        output = get_print(input_mock, [keyword, advanced_option, favorite])
        # Expected output indicating the favorite article 'Rock music' is present in the search results.
